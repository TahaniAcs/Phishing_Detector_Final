...
# ----------------- 1. Feature Extraction Functions (ML Core) -----------------
...
# ----------------- 2. Model Training (Executed once) -----------------
@st.cache_resource
...
def main():
    ...
    # Custom CSS for aesthetics (Simplified for robust deployment)
    st.markdown("""
        <style>
        ...
        /* Style for the Purple Metric Boxes */
        .purple-box {
    ...
    # -------------------------------------------------------------
    # LEFT COLUMN: Header, Input, and Button
    # -------------------------------------------------------------
    with col_input:
        ...
        # Removed image here for guaranteed deployment success
        st.markdown("<p style='text-align: center; color: #888888; font-size: 14px;'>Run your link analysis below.</p>", unsafe_allow_html=True)


        # Input (using unique key)
        default_url = "http://secure.paypal.com@verify-account.info/login.html"
        user_url = st.text_input("Enter URL to Analyze:", default_url, key="url_input_field_live")
    ...
    with col_analysis:
        ...
        if st.session_state.get('analysis_started_live', False):
...
if __name__ == '__main__':
    ...
    if 'analysis_started_live' not in st.session_state:
        st.session_state['analysis_started_live'] = False
